...
- вывод распознанного ЦС-текста шрифтом Ponomar Unicode.
- бинаризация строк перед подачей в CRNN.
...
Кнопка `Выбрать изображение страницы` доступна и в симуляторе, и на реальном iPhone. В iOS Simulator системный document scanner недоступен, поэтому там нужно пользоваться выбором изображения из медиатеки.
...
Изображение строки масштабируется до высоты `48 px`, сохраняет пропорции, дополняется белым фоном до ширины `1024 px`, затем бинаризуется перед подачей в CRNN.
